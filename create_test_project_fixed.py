#!/usr/bin/env python3
"""
Create test project and data for LNGT TURKIYE (REGAS MODEL) - Fixed version
"""
import sys
import os
    ...
    try:
        ...
        for i in range(1, 101):  # 100 joints
            joint = StructureMasterJointList(
                ...
                drawing_rev=f"Rev {random.randint(0, 3)}",
                ...
                thickness=str(random.uniform(5, 25)),
        ...
        for i in range(1, 51):  # 50 materials
            material = StructureMaterialRegister(
                ...
                grade=random.choice(['ASTM A36', 'ASTM A572', 'ASTM A992']),
                thickness=str(random.uniform(5, 25)),
                length=str(random.uniform(1000, 6000)),
                width=str(random.uniform(100, 500)),
        ...
        for i, joint in enumerate(joints[:60]):  # 60 fit-ups
            fitup = StructureFitUpInspection(
                ...
                updated_by=random.choice(['John Doe', 'Jane Smith', 'Bob Johnson']),
        ...
        for i, fitup in enumerate(fitups[:24]):  # 40% of 60 = 24 finals
            final = StructureFinalInspection(
                ...
                weld_length=random.uniform(50, 500),
                weld_site=random.choice(['shop', 'field']),
                remarks="Test final inspection",
                inspection_category=fitup.inspection_category
            )
            session.add(final)
            final_created += 1
        ...
        for final in finals:
            ...
            for method, method_type, typical_length in selected_methods:
                ...
                ndt_test = NDTTest(
                    ...
                    tested_by=random.choice(['John Doe', 'Jane Smith', 'Bob Johnson'])
                )
        ...
        for final in finals:
            ...
            if tests:
                ...
                ndt_status = StructureNDTStatusRecord(
                    project_id=project.id,
                    final_id=final.id,
                    draw_no=final.draw_no,
                    structure_category=final.structure_category,
                    page_no=final.page_no,
                    ...
                    inspection_category=final.inspection_category
                )
        ...
        for final in finals:
            ndt_request = StructureNDTRequest(
                ...
                incharge_person=random.choice(['John Doe', 'Jane Smith', 'Bob Johnson'])
            )
        ...
        for i in range(1, 21):  # 20 WPS
            wps = WPSRegister(
                project_id=project.id,
                wps_no=f"WPS-{i}",
                material_group=random.choice(['ASTM A36', 'ASTM A572', 'ASTM A992']),
        ...
        for i in range(100, 120):  # 20 welders
            welder = WelderRegister(
                project_id=project.id,
                welder_no=f"WELD-{i}",
                welder_name=random.choice(['John Smith', 'Jane Doe', 'Bob Wilson', 'Alice Johnson', 'Charlie Brown']),
                qualified_material=random.choice(['ASTM A36', 'ASTM A572', 'ASTM A992']),
                thickness_range=f"{random.uniform(5, 10):.1f}-{random.uniform(20, 25):.1f} mm",
                weld_process=random.choice(['SMAW', 'GMAW', 'FCAW', 'GTAW']),
                qualified_position=random.choice(['1G', '2G', '3G', '4G']),
                status=random.choice(['active', 'inactive'])
            )
            session.add(welder)
            welders_created += 1
